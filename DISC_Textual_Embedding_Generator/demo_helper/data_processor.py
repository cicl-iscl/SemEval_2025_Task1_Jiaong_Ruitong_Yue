...
class DataHandler(object):
    ...
    def get_glove_and_pos_and_chartokens(self, sentences):
        ...
        for sent in sentences:
            ...
            source_sentence_glove_tknz = [
                int(self.glove_vocab[t]) if t in self.glove_vocab else int(self.glove_vocab['<UNK>']) for t in
                source_sentence_glove_words]
            source_sentence_pos_taggs = [int(self.pos_vocab[t]) if t in self.pos_vocab else int(self.pos_vocab['<UNK>'])
                                         for t in source_sentence_pos_taggs]
            ...
            for word in source_sentence_glove_words:
                if word in ['<S>', '<E>', '<UNK>']:
                    ...
                    source_sentence_char_tknz.append(
                        [int(self.char_vocab[w]) if w in self.char_vocab else int(self.char_vocab['<UNK>']) for w in
                         list(word)])
    ...
    def get_pie_position(self, sentences, pies):
        """
        ...
        """
        pie_pos = []
        for sen, p in zip(sentences, pies):
            encoded = self.tokenizer(sen, return_offsets_mapping=True, truncation=True,
            ...
            pie_pos.append((start_idx, end_idx))
        return pie_pos

    def prepare_input(self, sentences, pies):
        """
        ...
        pies = [apply_contraction_change(p) for p in pies]
        ...
        pie_positions = self.get_pie_position(sentences, pies)
