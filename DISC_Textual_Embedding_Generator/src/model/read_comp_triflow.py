...
class ReadingComprehensionDetector(nn.Module):
    ...
    def forward(self, data):
        ...
        #
        pie_positions = data['pie_positions']
        ...
        batch_size = M_1.size(0)
        pie_embeddings = []
        for i in range(batch_size):
            start_idx, end_idx = pie_positions[i]
            pie_embed = M_1[i, start_idx:end_idx + 1, :]
            pie_embed = pie_embed.mean(dim=0)
            pie_embeddings.append(pie_embed)
        pie_embeddings = torch.stack(pie_embeddings)
